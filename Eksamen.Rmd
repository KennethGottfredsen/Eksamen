...
Da antallet af variabler i det samlede datasæt er mindre end antallet af observationer, benyttes backward-selection til, at udvælge de uafhængige variabler som fremadrettet skal indgå i de forskellige modeller.
...
Baseline modellen er den simpleste uden uafhængige variabler. Her er den gennemsnitlige efterspørgsel den afhængige variabel. Baselinemodellen har MSE = $19.376$, det er meget langt fra 0.  
...
```{r, Chunk 16 - Sammenligning af MSE, include=TRUE, eval= FALSE, warning=FALSE}

Metode_LOOCV = c("Testmodel", "Testmodel", "Testmodel", "Testmodel",                            "Træningsmodel", "Træningsmodel", "Træningsmodel",                             "Træningsmodel")
Model_kompleksitet = c("Baseline", "Simpel", "Mellem", "Kompleks","Baseline",                         "Simpel", "Mellem", "Kompleks")
MSE_Tusinde = c(19.376, 16.993, 7.841, 7.987, 19.635, 16.576, 6.740, 6.923)
test_frame = data.frame(Metode_LOOCV, Model_Kompleksitet, MSE_Tusinde,
                            stringsAsFactors = TRUE)
test_frame$Model_Kompleksitet <- factor(test_frame$Model_Kompleksitet,
                      levels = c("Baseline", "Simpel", "Mellem", "Kompleks"))
MSE_plot <- ggplot(test_frame) + 
geom_bar(aes(x = Model_Kompleksitet, y = MSE_Tusinde, fill=Metode_LOOCV), 
