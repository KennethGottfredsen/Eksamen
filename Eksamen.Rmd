...
Derefter laver vi en ggplot for at se fordelingen af efterspørgselen af koldskål i form af et histogram. 

Vi bruger geom_density til at forstå fordelingen og til at forudsige den
forventede fordeling af efterspørgslen på koldskål. 
Man kan se at at spredningen af observationerne er størst omkring 500 liter koldskål. Endvidere kan man observere, at efterspørgslen af koldskål er 
forholdsvis normalfordelt, og at sandsynlighedskurven er klokkeformet.  
...
ggplot(data3, aes(x = efterspørgsel)) +
 ...
 geom_density(alpha=0.5, fill="#FF6666", adjust=1.5) +
  ...
  xlim(129, 850) + ylim(0, 0.0035)

```

På følgende kode-chunk har vi lavet et boxplot til at vise den statistiske
variationen ift. butikkens forventede lagerbeholdning og efterspørgslen på koldskål. 
...
hænger moderat sammen med sommerperioden. Bemærk også at kattehårene på boksplottene er forholdsvis brede, dette indikerer fx. at observationerne ligger langt væk fra medianen hvilket øger den statistiske usikkerhed. 
...
cor_matrice <- data3 |>  
  dplyr::select(efterspørgsel, temp_min_past1h,
                temp_dry, temp_max_past1h,                                                     temp_mean_past1h) 
...
glimpse(data3)
# + I(temp_mean^8)
fit <- lm(efterspørgsel ~., data = data3) # kompleks model med alle variable. 
summary(fit)

fit2 <- lm(efterspørgsel ~ dag, data = data3) 
summary(fit2)



attach(data3)
lm.fit1 = lm(efterspørgsel ~ forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + I(temp_mean_past1h^1))
...
lm.fit2 <- lm(efterspørgsel ~ poly(temp_mean_past1h, degree = 4), data = data3)
summary(lm.fit2)
...
glm.fit2 <- glm(efterspørgsel~. -temp_mean_past1h^2, data = data3) # ekstrem model i første grad 
cv.err2 <- cv.glm(data3, glm.fit2)
cv.err2$delta[[1]]

glm.fit3 <- glm(efterspørgsel ~ . + I(temp_mean_past1h^2) -humidity_past1h, data = data3) # Ekstrem model i anden grad.
cv.err3 <- cv.glm(data3, glm.fit3)
cv.err3$delta[[1]]
summary(glm.fit3)
...
glm.fit3 <- glm(efterspørgsel ~ + forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + temp_mean_past1h, data = data3) 
cv.err3 <- cv.glm(data3, glm.fit3) # Mellem. 
cv.err3$delta[[1]]
summary(glm.fit3)


glm.fit3 <- glm(efterspørgsel ~ + forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + I(temp_mean_past1h^3), data = data3) 
cv.err3 <- cv.glm(data3, glm.fit3) # Kompleks model overfitter, da MSE stiger. 
cv.err3$delta[[1]]
summary(glm.fit3)


glm.fit3 <- glm(efterspørgsel ~ + forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + I(temp_mean_past1h^22), data = data3) 
cv.err3 <- cv.glm(data3, glm.fit3)
cv.err3$delta[[1]]
summary(glm.fit3)
...
cv.error <- rep(0, 5)
for (i in 1:5) {
  glm.fit <- glm(y~poly(x , i), data = data3)
...
cv.error1 <- rep(0, 10)
for (i in 1:10) {
  glm.fit1 <- glm(efterspørgsel ~ poly(temp_mean_past1h + temp1 + temp2 + temp3, i), data = data3)
...
for (i in 1:5) {
  ...
  cv.error1[i] <- cv.glm(data3, glm.fit2)$delta[1]
}
cv.error1
summary(glm.fit2)
...
cv.error1 <- rep(0, 10)
for (i in 1:10) {
  glm.fit3 <- glm(efterspørgsel ~ . + I(temp_max_past1h^2) -humidity_past1h, data = data3)
  cv.error1[i] <- cv.glm(data3, glm.fit1)$delta[1]
}
cv.error1
summary(glm.fit1)
