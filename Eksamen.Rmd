...
ggplot(data = data3, mapping = aes(x = forvent_lager, y = efterspørgsel, fill =
  ...
  theme( plot.title = element_text(hjust = 0.5, size = 16),
         plot.subtitle = element_text(hjust = 0.5, size = 14), 
         plot.caption = element_text(hjust = 2.5, face = "italic",size = 10 )) +
...
Eftersom antallet af variabler i vores datasæt er større end antallet af observationer, vil vi bruge backward-selection til, at udvælge de uafhængige variabler der skal med i modellen.
Det vil sige, at vi tilføjer alle variable ind på højre side af ligningen, og fjerner dem med den højeste p-værdi indtil der kun er signifikante uafhængige variable tilbage.
...
Derfor har vi valgt at bruge denne variabel som den uafhængige effekt i næste kodechunk. Som førnævnt var der en moderat signifikant sammenhæng mellem efterspørgslen og gennemsnits-temperaturen. 
...
Når modellen er blevet trænet godt igennem, bliver den afprøvet på testdata, da vi gerne vil undersøge hvor god modellen er til, at forudsige en så præcis efterspørgslen på koldskål som mulig. Vurderingen af modelpræcisionen bestemmes ud fra den laveste MSE værdi.

# Baseline model på træningsdata

Baseline modellen er den simpleste uden uafhængige variabler. Her er den gennemsnitlige efterspørgsel den afhængige variabel. Baselinemodellen har MSE = $19.376$ hvilket er meget langt fra 0.  

# Simpel model på træningsdata

Den simple model har en MSE på $16.576$. Den er bedre end baselinemodellen fordi vi har øget kompleksiteten ved, at tilføje temp_mean_past1h. 

# Mellem model på træningsdata

# Kompleks model på træningsdata
...
lm.fit3_træning = lm(efterspørgsel ~ forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + måned + I(temp_mean_past1h^1), data = data3)
vif(lm.fit3_træning) # VIF > 1 indikerer at der er inflation i variansen på alle variabler.
summary(lm.fit3_træning)
lm_fit3_summary <- summary(lm.fit3_træning)
mean(lm_fit3_summary$residuals^2) # MSE = 6740.342

# Kompleks model 

lm.fit3_træning = lm(efterspørgsel ~ forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + måned + I(temp_mean_past1h^22), data = data3)
vif(lm.fit3_træning) # VIF > 1 indikerer at der er inflation i variansen på alle variabler.
summary(lm.fit3_træning)
lm_fit4_summary <- summary(lm.fit3_træning)
mean(lm_fit4_summary$residuals^2) # MSE = 6923.087
```

## Modeludvælgelse og test på testsdata

Beskriv LOOCV metoden. 
...
glm.fit1 = glm(efterspørgsel ~ 1, data = data3) 
cv.err3 <- cv.glm(data3, glm.fit1) 
cv.err3$delta[[1]] # MSE = 19635.76
summary(glm.fit1)

# Simpel testmodel

glm.fit1 = glm(efterspørgsel ~ temp_mean_past1h, data = data3) 
cv.err3 <- cv.glm(data3, glm.fit1) 
cv.err3$delta[[1]] # MSE = 16993.9
summary(glm.fit1)

# Mellem testmodel

glm.fit2 <- glm(efterspørgsel ~ + forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + måned + temp_mean_past1h, data = data3) 
cv.err3 <- cv.glm(data3, glm.fit2)
cv.err3$delta[[1]] # MSE = 7841.251
summary(glm.fit2)

# Kompleks model

glm.fit3 <- glm(efterspørgsel ~ + forvent_lager + weekend_helligdag + kamjunk + temp_gt25_3_dage + måned +  I(temp_mean_past1h^22), data = data3) 
cv.err3 <- cv.glm(data3, glm.fit3) # Kompleks model overfitter, da MSE stiger. 
cv.err3$delta[[1]] # MSE = 7987.699
summary(glm.fit3)
```

På baggrund af vores MSE værdier, har vi opstillet et histogram der visuelt viser, hvordan MSE'en bliver mindre i takt med at modelkompleksiteten bliver større. MSE er somsagt et udtryk for hvor godt en model yder på træningsdata. Men den MSE der bliver beregnet ud fra træningsdata kan være biased, derfor beregnes der en MSE på testdata vha. LOOCV metoden. Nævn bias variance tradeoff her. 



```{r, Chunk 15 - Sammenligning af MSE, include=TRUE, eval= FALSE, warning=FALSE}

LOOCV_Metode <- c("Testmodel", "Testmodel", "Testmodel", "Testmodel", "Træningsmodel", "Træningsmodel", "Træningsmodel", "Træningsmodel")
Modelkompleksitet <- c("Baseline", "Kompleks", "Mellem", "Simpel","Simpel", "Mellem", "Kompleks", "Baseline")
MSE <- c(19376, 7987, 7841, 16993, 16576, 6740, 6923, 19635)
df_mse <- data.frame(LOOCV_Metode, MSE, Modelkompleksitet)
print (df_mse)
str(df_mse)

ggplot(df_mse) + 
geom_bar(aes(x=Modelkompleksitet, y=MSE, fill=LOOCV_Metode), 
stat="identity", # Don't transform the data
position = "dodge") + 
labs(title = "Sammenligning af trænings og test MSE")
...
LOOCV = c("Testmodel", "Testmodel", "Testmodel", "Testmodel", "Træningsmodel", "Træningsmodel", "Træningsmodel", "Træningsmodel")
Kompleksitet = c("Baseline", "Kompleks", "Mellem", "Simpel","Simpel", "Mellem", "Kompleks", "Baseline")
MSE_test = c(19376, 7987, 7841, 16993, 16576, 6740)

# Keep strings as characters
test_frame = data.frame(LOOCV, Kompleksitet, MSE,
                            stringsAsFactors = TRUE)
test_frame$Kompleksitet < - factor(test_frame$Kompleksitet,
                      levels = c("B", "D", "A", "C"))
str(test_frame)
levels(test_frame$Kompleksitet)
attributes(test_frame)
glimpse(student_frame)

unique(test_frame$Kompleksitet)

ggplot(df_mse) + 
geom_bar(aes(x=Kompleksitet, y=MSE, fill=LOOCV), 
stat="identity", # Don't transform the data
position = "dodge") + 
labs(title = "Sammenligning af trænings og test MSE")
