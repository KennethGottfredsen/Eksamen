...
pacman::p_load("tidyverse", "magrittr", "nycflights13", "gapminder",
               ...
               "ggbeeswarm", "palmerpenguins", "hms", "RcolorBrewer", "boot", "openxlsx", "writexl", "pacman")
```
...
glm.fit <- glm(efterspørgsel ~ poly(temp_max_past1h + temp_max_past1h^5))
summary(glm.fit)
...
ggplot(data3, mapping = aes(x=x, y=y)) + 
  geom_point(alpha=1/3) +
  geom_smooth(method="lm", formula = y ~ poly(x, 1, raw=TRUE), se=FALSE, colour="blue") +
  geom_smooth(method="lm", formula = y ~ poly(x, 2, raw=TRUE), se=FALSE, colour="red") +
  geom_smooth(method="lm", formula = y ~ poly(x, 22, raw=TRUE), se=FALSE, colour="green") +
geom_point(data=data3, mapping = aes(x=x, y=y), alpha=1/3)
```
