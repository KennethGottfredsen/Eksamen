...
Req_url <-  "stationId=06186&datetime=2022-04-01T12%3A00%3A00Z%2F2022-08-
30T12%3A00%3A00Z&api-key=1fda8cc2-25bf-45e3-9f43-702fb9ccfdf3&limit=80182" 
...
3 dage i træk med >= 25 grader. Det er en dummyvariabel da vi bruger if_else. 
...
cv.error1 <- rep(0, 10)
for (i in 1:10) {
  glm.fit1 <- glm(efterspørgsel ~ poly(temp_max_past1h + temp_min_past1h + humidity + temp_dry + temp_dew + humidity_past1h + temp_mean_past1h , i), data = data3)
...
pacman::p_load("tidyverse", "magrittr", "nycflights13", "gapminder",
               ...
               "fueleconomy", "viridis", "readxl", "timeDate", "tinytex",
               "ggbeeswarm", "palmerpenguins", "hms", "RColorBrewer", "boot",
               "openxlsx", "writexl", "pacman", "corrplot")
```
...
# 3 Først bruger vi base_url og info_url til at anmode om vejrdata fra DMI's API. req_url bruges til at udvælge specifikke parametre fra api´en. 
...
ggplot(data3, aes(x = efterspørgsel)) + 
  ...
  geom_density(alpha=0, fill="#FF6666", adjust=1.5) + # Standardiseret så arealet = 1 
  labs(title = "Histogram over butikkernes efterspørgsel af koldskål",
       subtitle = "Undersøger om efterspørgslen er normaltfordelt",
       y = "Antal observationer",
       x = "Butikkernes efterspørgsel af koldskål",
...
lm.fit2 = glm(efterspørgsel ~ 1) # simpel model
summary(lm.fit2)
